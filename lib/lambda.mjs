...
import { getApiGatewayReqTime, getContextLogStreamTime } from './utils.mjs';

const logStreamId = randomBytes(16).toString('hex');
...
export const events = {
  ...
  async 'LAMBDA_FUNCTION_EVENT'(req) {
    ...
    return {
      ...
      rawPath,
      rawQueryString,
      ...(getReqHeader(req, 'Cookie') ? { cookies: getReqHeader(req, 'Cookie').split(';').map(s => s.trim()) } : null),
      headers: { ...req.headers },
  ...
  async 'API_GATEWAY_PROXY_EVENT_1.0'(req) {
    ...
    return {
      ...
      requestContext: {
        ...
        domainName: `${apiId}.execute-api.${region}.amazonaws.com`,
        domainPrefix: apiId,
        requestTime: getApiGatewayReqTime(now),
...
export const contexts = {
  ...
  'LAMBDA_CONTEXT'() {
    ...
    return {
      ...
      logStreamName: `${getContextLogStreamTime(now)}/[${functionVersion}]${logStreamId}`,
